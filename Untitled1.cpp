...
//void sellsman::save_sellstosellsMant()
//{
//
//   ofstream file;
//   if(rakib==true){file.open("rakib.txt",ios::out|ios::app|ios::in);}
//   else if(rimon==true){file.open("rimon.txt",ios::out|ios::app|ios::in);}
//   else if(alvi==true){file.open("alvi.txt",ios::out|ios::app|ios::in);}
//   else {file.open("arif.txt",ios::out|ios::app|ios::in);}
...
//   }
//}
//void sellsman::show_selltodaySellsM(int n)
//{
//ifstream file;
//    switch (n){
//  case 1:
...
//    while(!file.eof()){
//     cout<<brand<<setw(15)<<sizeOrType<<setw(13)<<price<<endl;
//    }
//}
//
...
class product{
  public:
    ...
    void showOneForDateWise();
    void show_oneProduct();
    void update_product(int n,int s);
    int product_upMenu();
    void product_quantityMinus(int n,int s);
...
void product::product_quantityMinus(int n,int s)
{   int num=n;
    int serial=s;
    int a,b;
...
void product::showOneForDateWise()
{
    cout<<productSerial<<"."<<brand<<setw(8)<<"|"<<setw(9)<<sizeOrType<<setw(9)<<"|"<<setw(9)<<price<<endl;
}
void product::showDateWiseSell(int n,int m)
{ ifstream file;
  bool found=false;
file.open("datewisesell.txt",ios::in|ios::out);
if(!file.is_open()){cout<<"file is not opened";}
else {file.read((char*)this,sizeof(*this));
      while(!file.eof()){
        if(month==m&&day==n){ found=true;
        showOneForDateWise();
        file.read((char*)this,sizeof(*this));}
}}
  if(found==false){cout<<"DATE MISMATCH!!\n\n";}
}
...
case 1:
    ...
    if(choice3==1){goto mainmenu;}
     ...
     { p.getInvoiceProduct();
       p.dateWiseSell();
       p.product_quantityMinus(choice2,choice6);
       ...
       if(choice5==1){cout<<"Enter your Phone Number: ";
         ...
         remove("Invoice.txt");
         system("cls");
         cout<<"SUCCESSFULLY printed invoice\n";
...
case 3:
    ...
    cout<<"\n\n1.Main Menu     2. search again\n";
    cin>>choice1;
    system("cls");
    if(choice1==1){goto mmenu;}
    else {goto searchd;}
    break;
 ...
 {   agn:
   ...
   char ch,ch1;
    ...
    while(ch1 != 13){
       ...
       cout << "\nAccess granted :P\n";
        if(pass=="Rakib"||"rakib"){rakib=true;}
          else if(pass=="Rimon"||"rimon"){rimon=true;}
         else if(pass=="Arif"||"arif"){arif=true;}
          else if(pass=="Alvi"||"alvi"){rakib=true;}
       }
